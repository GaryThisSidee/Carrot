...
def send():
    ...
    if form.validate_on_submit():
        success = current_user.send(form.quantity.data, form.receiver.data,
                                    form.note.data)
...
def create():
    ...
    if form.validate_on_submit():
        try:
            asset_id = current_user.create(form.asset_name.data,
                                           form.unit_name.data,
                                           form.total.data, form.decimals.data,
                                           form.default_frozen.data,
                                           form.url.data)

            return render_template('error_1.html', success=True)
        except:
            return render_template('error_1.html', success=False)
    # show the form, it wasn't submitted
    return render_template('create_asset.html', form=form)


@main_bp.route('/createe', methods=['GET', 'POST'])
@login_required
def createe():
    form = NFTForm()
    if form.validate_on_submit():
        try:
            ...
            asset_id = current_user.createee(file, form.asset_name.data,
                                             form.unit_name.data,
                                             form.url.data)
            return render_template('error.html', success=True)
        except:
            return render_template('error.html', form=form)
    # show the form, it wasn't submitted
    return render_template('create_nft.html', form=form)
